    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            if (counter == 3 && NeedTime) {
                ...
                if(counter == 99){
                    robot.Drive(.9,-40,telemetry,3);
                    counter = 5;
                }
                if(counter == 4){
                    robot.Drive(.5,-60,telemetry,3.5);
                    counter ++;
                }
                if(counter == 5){
                    robot.EncoderTurn(Robot.Direction.ClockWise,.6,130,3);
                    counter ++;
                }
                if(counter == 6){
                    robot.GBR.setPower(RobotConstants.Suckers_In);
                    robot.GBL.setPower(RobotConstants.Suckers_In);
                    distance = robot.DriveWithSuck(.95,100,telemetry,5);
                    robot.GBR.setPower(RobotConstants.Suckers_Stay);
                    robot.GBL.setPower(RobotConstants.Suckers_Stay);
                    counter ++;
                }
                if(counter == 7){
                    robot.Drive(.9,-distance/RobotConstants.Tickspercm,telemetry,4);
                    counter ++;

                }
                if(counter == 8){
                    robot.EncoderTurn(Robot.Direction.ClockWise,.6,75,3);
                    counter ++;
                }
                if(counter == 9){

                    if(VuMarkOutput == RelicRecoveryVuMark.RIGHT){
                        robot.Drive(.9,-10,telemetry,3);
                        counter ++;
                    }
                    if(VuMarkOutput == RelicRecoveryVuMark.CENTER){
                        robot.Drive(.9,11,telemetry,3);
                        counter ++;

                    }

                    if(VuMarkOutput == RelicRecoveryVuMark.LEFT){
                        robot.Drive(.9,32,telemetry,5);
                        counter ++;
                    }
                }
                if(counter == 10){
                    ...
                    robot.EncoderTurn(Robot.Direction.ClockWise,.6,80,3);
